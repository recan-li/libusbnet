...
# 30 "src/proto/protocol.c" 2



# 32 "src/proto/protocol.c"
static pthread_mutex_t __mutex = 
# 32 "src/proto/protocol.c" 3 4
                                { { 0, 0, 0, 0, 0, 0, 0, { 0, 0 } } }
# 32 "src/proto/protocol.c"
                                                         ;


static Packet* sPacket = 
# 35 "src/proto/protocol.c" 3 4
                        ((void *)0)
# 35 "src/proto/protocol.c"
                            ;
...
# 41 "src/proto/protocol.c" 3 4
     stderr
# 41 "src/proto/protocol.c"
     , "%s: invalid packet size (size = 0)", __func__); fprintf(
# 41 "src/proto/protocol.c" 3 4
     stderr
# 41 "src/proto/protocol.c"
     , "\n"); }; } while(0);
      return 
# 42 "src/proto/protocol.c" 3 4
            ((void *)0)
# 42 "src/proto/protocol.c"
                ;
   }
...
int pkt_reserve(Packet* pkt, uint32_t size)
{
   if(pkt->bufsize < size) {
      pkt->bufsize = size + 32;
      if((pkt->buf = realloc(pkt->buf, pkt->bufsize)) == 
# 71 "src/proto/protocol.c" 3 4
                                                        ((void *)0)
# 71 "src/proto/protocol.c"
                                                            ) {
         do { if(log_level() & MsgError) { fprintf(
# 72 "src/proto/protocol.c" 3 4
        stderr
# 72 "src/proto/protocol.c"
        , "%s: failed to allocate packet buffer (size = %u)", __func__, size); fprintf(
# 72 "src/proto/protocol.c" 3 4
        stderr
# 72 "src/proto/protocol.c"
        , "\n"); }; } while(0);
   ...
   return pkt->buf != 
# 77 "src/proto/protocol.c" 3 4
                     ((void *)0)
# 77 "src/proto/protocol.c"
                         ;
}
...
Packet* pkt_claim() {
   ...
   if(sPacket == 
# 90 "src/proto/protocol.c" 3 4
                ((void *)0)
# 90 "src/proto/protocol.c"
                    ) {
      sPacket = pkt_new(32, 0x00);
   }
...
uint32_t pkt_recv(int fd, Packet* dst)
{
   ...
   if(!pkt_reserve(dst, (sizeof(uint8_t)+sizeof(uint8_t)+sizeof(uint32_t))))
      return 0;

   if((size = pkt_recv_header(fd, dst->buf)) == 0) {
      do { if(log_level() & MsgError) { fprintf(
# 114 "src/proto/protocol.c" 3 4
     stderr
# 114 "src/proto/protocol.c"
     , "%s: failed to receive packet header", __func__); fprintf(
# 114 "src/proto/protocol.c" 3 4
     stderr
# 114 "src/proto/protocol.c"
     , "\n"); }; } while(0);
   ...
   if(dst->size > 0) {
      ...
      if((dst->size = recv_full(fd, dst->buf, dst->size)) == 0) {
         do { if(log_level() & MsgError) { fprintf(
# 131 "src/proto/protocol.c" 3 4
        stderr
# 131 "src/proto/protocol.c"
        , "%s: failed to receive packet payload", __func__); fprintf(
# 131 "src/proto/protocol.c" 3 4
        stderr
# 131 "src/proto/protocol.c"
        , "\n"); }; } while(0);
...
int pkt_send(Packet* pkt, int fd)
{
   ...
   char buf[(sizeof(uint8_t)+sizeof(uint8_t)+sizeof(uint32_t))] = { pkt->op };
   send(fd, buf, 1, 0);
...
int pkt_addnumeric(Packet* pkt, uint8_t type, uint16_t len, int32_t val)
{
   ...
   uint8_t val8 = val;
   uint16_t val16 = 
# 193 "src/proto/protocol.c" 3 4
                   (__extension__ ({ unsigned short int __v, __x = (unsigned short int) (
# 193 "src/proto/protocol.c"
                   (uint16_t) val
# 193 "src/proto/protocol.c" 3 4
                   ); if (__builtin_constant_p (__x)) __v = ((unsigned short int) ((((__x) >> 8) & 0xff) | (((__x) & 0xff) << 8))); else __asm__ ("rorw $8, %w0" : "=r" (__v) : "0" (__x) : "cc"); __v; }))
# 193 "src/proto/protocol.c"
                                        ;
   val = 
# 194 "src/proto/protocol.c" 3 4
        __bswap_32 (
# 194 "src/proto/protocol.c"
        val
# 194 "src/proto/protocol.c" 3 4
        )
# 194 "src/proto/protocol.c"
...
void* pkt_begin(Packet* pkt, Iterator* it)
{
   ...
   it->type = InvalidType;
   it->len = 0;
   it->cur = it->next = it->end = 
# 214 "src/proto/protocol.c" 3 4
                                 ((void *)0)
# 214 "src/proto/protocol.c"
                                     ;
...
void* iter_next(Iterator* it)
{
   ...
   if(it->next >= it->end) {
      it->cur = it->next;
      return 
# 238 "src/proto/protocol.c" 3 4
            ((void *)0)
# 238 "src/proto/protocol.c"
                ;
   }
